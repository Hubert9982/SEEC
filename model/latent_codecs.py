...
class SlimChannelGroupsLatentCodec(LatentCodec):
    """Reconstructs groups of channels using previously decoded groups.
    ...
    def forward(self, y: Tensor, side_params: Tensor) -> Dict[str, Any]:
        ...
        for k in range(len(self.groups)):
            self.set_activate_channels(k)
            ctx_params = self.channel_context(y_hat_, k)  # modified # TODO modify compress decompress
            y_out_[k] = self.latent_codec(y_[k], ctx_params, side_params)  # modified # TODO modify compress decompress
            y_hat_[k] = y_out_[k]["y_hat"]
            y_likelihoods_[k] = y_out_[k]["likelihoods"]["y"]
    ...
    def set_groups(self, groups: List[int]) -> None:
        self.groups = list(groups)
        self.channel_context.init_activate_channels(self.groups)

    def set_activate_channels(self, k):
        self.channel_context.set_activate_channels(k)
